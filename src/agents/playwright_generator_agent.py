...
Reads:
  - output/testcase_generated/<stem>_raw_output.json (from testcase_agent), or
...
Usage (from any cwd; paths resolved from repo root):
  python -m src.agents.playwright_generator_agent output/testcase_generated/payment_checkout_raw_output.json

  # If output/payment_checkout_testcases.json does not exist, the same stem is tried
  # under output/testcase_generated/payment_checkout_raw_output.json (testcase_agent output).
  python -m src.agents.playwright_generator_agent output/payment_checkout_testcases.json
"""
...
# Default output from testcase_agent (see src/agents/testcase_agent.py)
...
def resolve_testcase_path(user_path: Path) -> Path:
    ...
    try the same locations testcase_agent writes (e.g. *_raw_output.json).
    """
    ...
    logger.error(
        "Test case JSON not found.\nLooked for:\n%s\n\nRun testcase_agent first, e.g.:\n"
        "  python -m src.agents.testcase_agent data/requirements/<name>.md\n"
...
def _playwright_config_ts() -> str:
    return """// playwright.config.ts — generated by playwright_generator_agent v1 (first run only)
...
def _base_fixture_ts() -> str:
    return """// fixtures/base.fixture.ts — generated by playwright_generator_agent v1 (first run only)
...
def main() -> None:
    ...
    try:
        ...
        logger.info("playwright_generator_agent — test cases: %s", source_tc)
